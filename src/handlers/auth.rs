...
pub async fn refresh(
    ...
    let token_version = db::users::get_user_token_version(&state.db, stored_token.user_id)
        .await?
        .ok_or_else(|| AppError::Unauthorized("Invalid refresh token".to_string()))?;
    let access_token =
        generate_access_token(stored_token.user_id, &state.jwt_secret, token_version)?;
